---
name: Pinpoint
slug: fahiem-pinpoint
author_slug: fahiem
categories: [form-field, infolist-entry]
description: Google Maps location picker component with search, draggable marker, reverse geocoding, radius support, street auto-fill, and multi-language support (EN, AR, NL, ID).
discord_url:
docs_url: https://raw.githubusercontent.com/fahiem152/filament-pinpoint/refs/heads/main/README.md
github_repository: fahiem152/filament-pinpoint
has_dark_theme: true
has_translations: true
versions: [4, 5]
publish_date: 2025-12-07
---
